...
L_TARGET = 3        # L-다양성 목표 값 (민감한 속성의 최소 다양성 수)
...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 4. 민감한 속성(연구주제) 생성: '입찰공고명'의 일부를 추출하여 민감한 속성으로 가정
    if "입찰공고명" in df.columns and "연구주제" not in df.columns:
        # 공고명의 앞 60자만 추출하여 민감한 속성으로 지정
...
def compute_l_diversity(df: pd.DataFrame, qi_cols: List[str], sensitive_col: str) -> pd.Series:
    """
    주어진 준식별자(QI) 컬럼 그룹별로 민감한 속성(Sensitive)의 고유값 개수(L-다양성)를 계산합니다.
    """
    # 준식별자(qi_cols) 기준으로 그룹화하고 민감한 속성의 고유값 개수(nunique)를 계산
...
def enforce_l_diversity_suppression(df: pd.DataFrame, qi_cols: List[str], sensitive_col: str, L_target: int) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    L-다양성 목표(L_target)를 충족하지 못하는 동질 집합의 민감한 속성 값을 'SUPPRESSED'로 억제합니다.
    """
    df = df.copy()
    # L-다양성 값 계산
    ...
    for ec_key in failing_ec:
        ...
        # 해당 동질 집합의 민감한 속성 값을 'SUPPRESSED'로 대체 (억제)
